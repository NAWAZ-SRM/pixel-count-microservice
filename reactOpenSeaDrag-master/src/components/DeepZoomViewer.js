...
const DeepZoomViewer = ({ tileSources, zoomLevel, xCoord, yCoord, annotDetArr, imageSettings, setViewer2, Doctor, tileName, widthTile, heightTile }) => {
  ...
  useEffect(() => {
    ...
    let image = { "Image": { "Format": "jpeg", "Overlap": 1, "Size": { "Height": heightTile, "Width":  widthTile}, "TileSize": 512, "Url": `http://34.28.249.83:5000/tile/${Doctor}/${tileName}/`, "xmlns": "http://schemas.microsoft.com/deepzoom/2008" }, "crossOriginPolicy": 'Anonymous', "ajaxWithCredentials": false, "useCanvas": true }
    ...
    anno.on('deleteAnnotation', async (annotation) => {
      ...
      await onFetchData('http://34.28.249.83:5000/deleteAnnotation', 'POST', { id: annotation.id })
    });

    anno.on("createAnnotation", async function (annotation) {


      await onFetchData('http://34.28.249.83:5000/getAnnotation', 'POST', annotation)
  ...
  const getSavedAnnotation = async () => {
    let savedData = await onFetchData('http://34.28.249.83:5000/getSavedAnnotation', 'GET',)
    return savedData;
  };
