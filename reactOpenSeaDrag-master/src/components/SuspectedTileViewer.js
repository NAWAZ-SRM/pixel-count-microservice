...
//         xywhxy: `xywhxy=pixel:${x.x1},${x.y1},${x.x2 - x.x1},${x.y2 - x.y1},${x.openSeaXCoord},${x.openSeaYCoord}`,
//         id: x.id,
//         title: x.title,
//         cat: x.cat
//       }));
...
//         y: x.top,  // Use top (pixel coordinate) instead of openSeaYCoord
//         annotation: x.title,
//         cat: x.cat,
//         title: x.title
//       }));
...
//       let annotDetArr = annotDet.Predicts.map(x => ({
//         xywhxy: `xywhxy=pixel:${x.x1},${x.y1},${x.x2 - x.x1},${x.y2 - x.y1},${x.left},${x.top}`, // Store left and top instead of openSeaXCoord, openSeaYCoord
//         id: x.id,
//         title: x.title,
//         cat: x.cat
//       }));
...
//         newImgArr = [...newImgArr, ...Images20];
//         setImages(newImgArr);
//         console.log('Final images array:', newImgArr.map(img => ({ id: img.id, title: img.title, x: img.x, y: img.y })));
//       }
//     };
...
//     console.log('Clicked tile data:', images.find(img => img.id === "1")); // For id "1" specifically
//     console.log('Clicked tile data:', images.find(img => img.annotation === annotation));
//     console.log('Clicked image:', {  x, y, zoom });
//     setSelectedAnnotation(annotation);
//     setShowDragonView(true);
//     setZoomLevel(50);
...
//                 src={image.src2}
//                 alt={image.title}
//                 onClick={() => handleImageClick(image.zoom, image.x, image.y, image.id)}  
//                 onContextMenu={(event) => onRightClick(event, image.id)}
//               />
//               <div className="absolute top-2 left-2 bg-black bg-opacity-50 text-white p-1 rounded-md text-xs">
//                 {image.title}
...
const SuspectedTileViewer = () => {
  ...
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = images.slice(indexOfFirstItem, indexOfLastItem); // Reverted to original
  ...
  useEffect(() => {
    ...
    const setupScreen = async () => {
      ...
      let annotDetArr = annotDet.Predicts.map(x => ({
        xywhxy: `xywhxy=pixel:${x.x1},${x.y1},${x.x2 - x.x1},${x.y2 - x.y1},${x.left},${x.top}`,
        id: x.id,
        title: x.title,
        cat: x.cat
      }));
  ...
  const handleImageClick = (zoom, x, y, id) => {
    if (scaleSelected) {
      return;
    }
    const clickedIndex = images.findIndex(img => img.id === id);
    const nextIndex = clickedIndex + 1; // Get the next image
    if (nextIndex >= images.length) {
      console.log('No next image available');
      return; // Prevent out-of-bounds
    }
    const nextImage = images[nextIndex];
    console.log('Clicked thumbnail:', { id, title: images[clickedIndex].title, x, y });
    console.log('Opening DeepZoomViewer with next image:', { id: nextImage.id, title: nextImage.title, x: nextImage.x, y: nextImage.y });
    setSelectedAnnotation(nextImage.id);
    setShowDragonView(true);
    setZoomLevel(35); // Or use nextImage.zoom if intended
    setXCoord(nextImage.x);
    setYCoord(nextImage.y);
  };
