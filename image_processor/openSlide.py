...
# def initiateTile(Doctor,DateFolder ,Patient):
#     """
...
def initiateTile(Doctor, Patient, DateFolder=None):
    """
    Initialize OpenSlide with the given Doctor, Patient, and optional DateFolder details.

    Args:
        Doctor (str): The doctor's folder name.
        Patient (str): The patient's folder name.
        DateFolder (str, optional): The date folder name. If None, uses the latest available.
    ...
    if not available_date_folders:
        raise FileNotFoundError(f"No date folders found for doctor: {Doctor}")
...
def tileSlide(Doctor, tileSlide, DateFolder=None):
    ...
    Args:
        Doctor (str): Doctor's name.
        tileSlide (str): Tile name (patient name).
        DateFolder (str, optional): Date folder name.

    Returns:
        Dictionary containing the predictions and tile details.
    """
    slide, width, height = initiateTile(Doctor, tileSlide, DateFolder)
    predict_list = get_box_list(Doctor, tileSlide, DateFolder)
    predictArr = []
...
def tile(Doctor, tileName, level, row, col, DateFolder=None):
    ...
    Args:
        Doctor (str): Doctor's name.
        tileName (str): Tile name (patient name).
        level (int): Zoom level.
        row (int): Row index.
        col (int): Column index.
        DateFolder (str, optional): Date folder name.
    ...
    """
    try:
        slide, width, height = initiateTile(Doctor, tileName, DateFolder)
...
def get_box_list(Doctor, tileSlide, DateFolder=None):
    ...
    X_Reference, Y_Reference = get_reference(Doctor, tileSlide, DateFolder)
    box_list = []
